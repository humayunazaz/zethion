export const getTranslation = {
  'COUNTRY.MNE': 'Montenegro',
  'COUNTRY.AFG': 'Afghanistan',
  'COUNTRY.WAL': 'Wales',
  'COUNTRY.GRN': 'Grenada',
  'COUNTRY.GRL': 'Greenland',
  'COUNTRY.YEM': 'Yemen',
  'COUNTRY.ENG': 'England',
  'COUNTRY.GRE': 'Greece',
  'COUNTRY.PCN': 'Pitcairn',
  'COUNTRY.SIN': 'Singapore',
  'COUNTRY.SJM': 'Svalbard And Jan Mayen',
  'REGISTRY.COMPETITION': 'Competition',
  'COUNTRY.GBR': 'Uk',
  'SITEMAP.DASHBOARD': 'Dashboard',
  'COUNTRY.BHU': 'Bhutan',
  'COUNTRY.MNP': 'Northern Mariana Islands',
  'COUNTRY.BHR': 'Bahrain',
  'REGISTRY.ROLE': 'Role',
  'COUNTRY.BIH': 'Bosnia Herzegovina',
  'COUNTRY.HUN': 'Hungary',
  'COUNTRY.TLS': 'East Timor',
  'COUNTRY.DMA': 'Dominica',
  'COUNTRY.SKN': 'Saint Kitts And Nevis',
  'LANGUAGE.FRA': 'French',
  'COUNTRY.FRA': 'France',
  'COUNTRY.OCE': 'Oceania',
  'COUNTRY.PUR': 'Puerto Rico',
  'COUNTRY.CHI': 'Chile',
  'COUNTRY.REU': 'Reunion',
  'LANGUAGE.ENG': 'English',
  'COUNTRY.DJI': 'Djibouti',
  'COUNTRY.BVT': 'Bouvet Island',
  'COUNTRY.PAR': 'Paraguay',
  'COUNTRY.KGZ': 'Kyrgyzstan',
  'COUNTRY.SGS': 'South Georgia And The Sandwich Islands',
  'COUNTRY.CHA': 'Chad',
  'COUNTRY.GPE': 'Guadeloupe',
  'COUNTRY.UKR': 'Ukraine',
  'COUNTRY.BFA': 'Burkina Faso',
  'COUNTRY.CXR': 'Christmas Island',
  'COUNTRY.PAN': 'Panama',
  'COUNTRY.UE': 'Europe',
  'COUNTRY.PAK': 'Pakistan',
  'COUNTRY.MLT': 'Malta',
  'COUNTRY.TJK': 'Tajikistan',
  'COUNTRY.IDN': 'Indonesia',
  'REGISTRY.GOALKEEPER': 'Goalkeeper',
  'SPORT.RACE': 'Race',
  'COUNTRY.MLI': 'Mali',
  'COUNTRY.CHN': 'China',
  'COUNTRY.PRK': 'North Korea',
  'REGISTRY.RIGHT': 'Right',
  'COUNTRY.AUT': 'Austria',
  'COUNTRY.AUS': 'Australia',
  'COUNTRY.RWA': 'Rwanda',
  'COUNTRY.CYP': 'Cyprus',
  'COUNTRY.SHN': 'St. Helena',
  'COUNTRY.TKM': 'Turkmenistan',
  'COUNTRY.KIR': 'Kiribati',
  'COUNTRY.TKL': 'Tokelau',
  'COUNTRY.GAM': 'Gambia',
  'COUNTRY.CIV': 'Ivory Coast',
  'COUNTRY.UMI': 'Minor Islands Of The United States Of America',
  'COUNTRY.NOR': 'Norway',
  'COUNTRY.CZE': 'Czech Republic',
  'COUNTRY.SYR': 'Syria',
  'COUNTRY.YUG': 'Yugoslavia',
  'COUNTRY.GAB': 'Gabon',
  'REGISTRY.PIVOT': 'Pivot',
  'COUNTRY.WORLD': 'World',
  'COUNTRY.MRI': 'Mauritius',
  'COUNTRY.MAR': 'Morocco',
  'COUNTRY.MAS': 'Malaysia',
  'COUNTRY.ERI': 'Eritrea',
  'REGISTRY.MALE': 'Female',
  'REGISTRY.LEFT': 'Left',
  'COUNTRY.SMR': 'San Marino',
  'COUNTRY.UAE': 'United Arab Emirates',
  'COUNTRY.BLZ': 'Belize',
  'COUNTRY.PHI': 'Philippines',
  'COUNTRY.MRT': 'Mauritania',
  'COUNTRY.BLR': 'Belarus',
  'COUNTRY.TPE': 'Taiwan',
  'LANGUAGE.USA': 'American',
  'REGISTRY.AMBIDEXTROUS': 'Ambidextrous',
  'COUNTRY.EST': 'Estonia',
  'LANGUAGE.ESP': 'Spanish',
  'COUNTRY.SAME': 'South America',
  'COUNTRY.ESP': 'Spain',
  'COUNTRY.USA': 'United States Of America',
  'COUNTRY.COL': 'Colombia',
  'COUNTRY.COM': 'Comore',
  'COUNTRY.COK': 'Cook Islands',
  'COUNTRY.ESH': 'Western Sahara',
  'COUNTRY.COD': 'Democratic Republic Of Congo',
  'COUNTRY.URU': 'Uruguay',
  'COUNTRY.ALB': 'Albania',
  'COUNTRY.ECU': 'Ecuador',
  'COUNTRY.SOL': 'Solomon Islands',
  'COUNTRY.SOM': 'Somalia',
  'COUNTRY.KOR': 'South Korea',
  'COUNTRY.ZMB': 'Zambia',
  'COUNTRY.MSR': 'Montserrat',
  'COUNTRY.NED': 'Netherlands',
  'COUNTRY.FRO': 'Faroe Islands',
  'COUNTRY.VAN': 'Vanuatu',
  'COUNTRY.ZIM': 'Zimbabwe',
  'COUNTRY.PER': 'Peru',
  'COUNTRY.NAM': 'Namibia',
  'REGISTRY.LATERAL': 'Lateral',
  'COUNTRY.MOZ': 'Mozambique',
  'REGISTRY.YEAR': 'Year',
  'COUNTRY.GUA': 'Guatemala',
  'COUNTRY.NRU': 'Nauru',
  'COUNTRY.SLE': 'Sierra Leone',
  'REGISTRY.COACH_PLAYER': 'Coach/Player',
  'COUNTRY.VAT': 'Vatican City',
  'COUNTRY.DOM': 'Dominican Republic',
  'COUNTRY.FSM': 'Federated States Of Micronesia',
  'COUNTRY.GUM': 'Guam',
  'COUNTRY.SLV': 'El Salvador',
  'COUNTRY.GUI': 'Guinea',
  'COUNTRY.EQG': 'Equatorial Guinea',
  'COUNTRY.GUF': 'French Guiana',
  'COUNTRY.AIA': 'Eel',
  'REGISTRY.LATERALITY': 'Laterality',
  'COUNTRY.NCL': 'New Caledonia',
  'COUNTRY.GER': 'Germany',
  'LANGUAGE.GER': 'German',
  'SPORT.QUALIFYING': 'Qualifying',
  'COUNTRY.MAD': 'Madagascar',
  'COUNTRY.GEO': 'Georgia',
  'COUNTRY.MAC': 'Macao',
  'COUNTRY.AZE': 'Azerbaijan',
  'COUNTRY.NCA': 'Nicaragua',
  'COUNTRY.TOG': 'Togo',
  'COUNTRY.GUY': 'Guyana',
  'COUNTRY.CMR': 'Cameroon',
  'REGISTRY.UNIVERSAL': 'Universal',
  'COUNTRY.ANT': 'Netherlands Antilles',
  'COUNTRY.SAU': 'Saudi Arabia',
  'COUNTRY.KAZ': 'Kazakhstan',
  'REGISTRY.PLAYER': 'Player',
  'REGISTRY.CENTER': 'Center',
  'COUNTRY.SRB': 'Serbia',
  'COUNTRY.CRO': 'Croatia',
  'COUNTRY.MEX': 'Mexico',
  'COUNTRY.IND': 'India',
  'COUNTRY.SAM': 'Samoa',
  'COUNTRY.ANG': 'Angola',
  'COUNTRY.CAY': 'Cayman Islands',
  'COUNTRY.ROU': 'Romania',
  'REGISTRY.COACH': 'Coach',
  'COUNTRY.LTU': 'Lithuania',
  'COUNTRY.SRI': 'Sri Lanka',
  'COUNTRY.JPN': 'Japan',
  'REGISTRY.SPORT': 'Sport',
  'COUNTRY.PLE': 'Occupied Palestinian Territories',
  'COUNTRY.FIJ': 'Fiji',
  'COUNTRY.HMD': 'Heard Island And Mcdonald Islands',
  'COUNTRY.MWI': 'Malawi',
  'COUNTRY.PLW': 'Palau',
  'COUNTRY.JAM': 'Jamaica',
  'COUNTRY.EGY': 'Egypt',
  'COUNTRY.NIR': 'Northern Ireland',
  'COUNTRY.LVA': 'Latvia',
  'COUNTRY.SCG': 'Serbia Montenegro',
  'COUNTRY.TUV': 'Tuvalu',
  'COUNTRY.BRB': 'Barbados',
  'COUNTRY.MGL': 'Mongolia',
  'COUNTRY.VIE': 'Vietnam',
  'COUNTRY.CTA': 'Central African Republic',
  'COUNTRY.ASIA': 'Asia',
  'COUNTRY.BRA': 'Brazil',
  'COUNTRY.TUR': 'Turkey',
  'COUNTRY.IOT': 'British Indian Ocean Territories',
  'COUNTRY.BAN': 'Bangladesh',
  'COUNTRY.LUX': 'Luxembourg',
  'COUNTRY.UGA': 'Uganda',
  'COUNTRY.DEN': 'Denmark',
  'COUNTRY.TUN': 'Tunisia',
  'COUNTRY.FIN': 'Finland',
  'COUNTRY.CCK': 'Cocos Islands',
  'COUNTRY.BAH': 'Bahamas',
  'LANGUAGE.IND': 'Hindi',
  'COUNTRY.GHA': 'Ghana',
  'COUNTRY.TAN': 'Tanzania',
  'COUNTRY.VEN': 'Venezuela',
  'COUNTRY.ALG': 'Algeria',
  'COUNTRY.LAO': 'Laos',
  'COUNTRY.NER': 'Niger',
  'COUNTRY.TAH': 'French Polynesia',
  'COUNTRY.ETH': 'Ethiopia',
  'COUNTRY.MCO': 'Monk',
  'COUNTRY.NEP': 'Nepal',
  'COUNTRY.NFK': 'Norfolk Island',
  'COUNTRY.SPM': 'Saint Pierre And Miquelon',
  'LANGUAGE.SAU': 'Arabic',
  'COUNTRY.MTQ': 'Martinique',
  'COUNTRY.MDA': 'Moldova',
  'COUNTRY.TRI': 'Trinidad And Tobago',
  'REGISTRY.FEMALE': 'Male',
  'LANGUAGE.JPN': 'Japanese',
  'COUNTRY.CPV': 'Cape Verde',
  'COUNTRY.BOT': 'Botswana',
  'COUNTRY.HKG': 'Hong Kong',
  'COUNTRY.LBY': 'Libya',
  'COUNTRY.GIB': 'Gibraltar',
  'COUNTRY.BOL': 'Bolivia',
  'COUNTRY.MDV': 'Maldives',
  'COUNTRY.LBR': 'Liberia',
  'COUNTRY.LBN': 'Lebanon',
  'COUNTRY.AND': 'Andorra',
  'COUNTRY.CAR': 'Caribbean',
  'COUNTRY.CRC': 'Costa Rica',
  'COUNTRY.TCA': 'Turks And Caicos Islands',
  'COUNTRY.CAN': 'Canada',
  'COUNTRY.VGB': 'British Virgin Islands',
  'COUNTRY.NAME': 'North America',
  'COUNTRY.CAM': 'Cambodia',
  'COUNTRY.JOR': 'Jordan',
  'COUNTRY.LCA': 'Saint Lucia',
  'COUNTRY.NGA': 'Nigeria',
  'COUNTRY.BDI': 'Burundi',
  'COUNTRY.SEY': 'Seychelles',
  'COUNTRY.ARU': 'Aruba',
  'COUNTRY.QAT': 'Qatar',
  'COUNTRY.IRN': 'Iran',
  'COUNTRY.FLK': 'Falkland Islands',
  'COUNTRY.IRL': 'Eire',
  'COUNTRY.ARM': 'Armenia',
  'COUNTRY.SEN': 'Senegal',
  'COUNTRY.UZB': 'Uzbekistan',
  'LANGUAGE.ITA': 'Italian',
  'COUNTRY.GNB': 'Guinea-Bissau',
  'COUNTRY.ASA': 'American Samoa',
  'COUNTRY.KVX': 'Kosovo',
  'COUNTRY.THA': 'Thailand',
  'COUNTRY.SVN': 'Slovenia',
  'LANGUAGE.CHN': 'Chinese',
  'COUNTRY.IRQ': 'Iraq',
  'COUNTRY.SVK': 'Slovakia',
  'COUNTRY.MKD': 'Fruit Salad',
  'COUNTRY.ISL': 'Iceland',
  'COUNTRY.SWE': 'Sweden',
  'COUNTRY.BUL': 'Bulgaria',
  'COUNTRY.ITA': 'Italy',
  'COUNTRY.SWZ': 'Swaziland',
  'COUNTRY.ATG': 'Antigua AND Barbuda',
  'COUNTRY.ATF': 'Southern French Territories',
  'COUNTRY.RUS': 'Russian Federation',
  'COUNTRY.HAI': 'Haiti',
  'LANGUAGE.RUS': 'Russian',
  'COUNTRY.BER': 'Bermuda',
  'COUNTRY.TIB': 'Tibet',
  'COUNTRY.BEN': 'Benin',
  'COUNTRY.CGO': 'Republic Of The Congo',
  'COUNTRY.ISR': 'Israel',
  'COUNTRY.KWT': 'Kuwait',
  'COUNTRY.LIE': 'Liechtenstein',
  'COUNTRY.BEL': 'Belgium',
  'COUNTRY.BRU': 'Brunei Darussalam',
  'COUNTRY.NZL': 'New Zeland',
  'COUNTRY.VIR': 'Us Virgin Islands',
  'COUNTRY.SCO': 'Scotland',
  'COUNTRY.LES': 'Lesotho',
  'COUNTRY.NIU': 'Niue',
  'COUNTRY.VIN': 'Saint Vincent And The Grenadines',
  'REGISTRY.TYPE': 'Type',
  'COUNTRY.MYA': 'Myanmar',
  'COUNTRY.CUB': 'Cuba',
  'COUNTRY.MHL': 'Marshall Islands',
  'COUNTRY.WLF': 'Wallis And Futuna',
  'COUNTRY.STP': 'Sao Tome And Principe',
  'COUNTRY.PNG': 'Papua New Guinea',
  'COUNTRY.HON': 'Honduras',
  'COUNTRY.SUI': 'Switzerland',
  'COUNTRY.SUD': 'Sudan',
  'COUNTRY.OMA': 'Oman',
  'COUNTRY.CUR': 'Curacao',
  'COUNTRY.RSA': 'South Africa',
  'LANGUAGE.POR': 'Portuguese',
  'COUNTRY.ARG': 'Argentine',
  'COUNTRY.KEN': 'Kenya',
  'COUNTRY.POR': 'Portugal',
  'COUNTRY.SUR': 'Suriname',
  'COUNTRY.TGA': 'Tonga',
  'COUNTRY.POL': 'Poland',
  'COUNTRY.MYT': 'Mayotte'};

export const getClusters = [
  "COMPETITION",
  "COUNTRY",
  "FORM",
  "LANGUAGE",
  "PLACE",
  "REGISTRY",
  "SITEMAP",
  "SOCIAL",
  "SPORT",
  "TEAM",
  "TestCluster",
  "USER"
]